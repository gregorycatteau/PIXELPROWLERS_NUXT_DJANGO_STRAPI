# 20 Product Specs/Ux Content — Index

> **Périmètre** : Documentation

<!-- GENERATED:START -->
<!-- ⚠️ Section générée automatiquement par ssot_generate_indexes.py — 2026-01-08 -->

## Documents

| Document | Status | Description |
|----------|--------|-------------|
| [P1_BLOCKS_NARRATIVE_TALIA.md](./P1_BLOCKS_NARRATIVE_TALIA.md) | active | P1 – Narration des bilans de blocs (Talia) |
| [P1_PANORAMA_NARATIVE_TALIA.md](./P1_PANORAMA_NARATIVE_TALIA.md) | active | P1 – Panorama · Narratives (Talia) |
| [PX_SKIP_SIGNAL_POLICY_V1.md](./PX_SKIP_SIGNAL_POLICY_V1.md) | active | PX Skip Signal Policy V1 |
| [PX_UNIVERSAL_QUESTIONNAIRE_UX_V1.md](./PX_UNIVERSAL_QUESTIONNAIRE_UX_V1.md) | active | PX Universal Questionnaire UX V1 |
| [PX_V1_3_1_P1_ATERRISSAGE_SYSTEMIQUE_COPY.md](./PX_V1_3_1_P1_ATERRISSAGE_SYSTEMIQUE_COPY.md) | active | P1 V1.3.1 — Atterrissage systémique (copy canon) |
| [PX_V1_3_1_P1_HYPOTHESES_STRUCTURANTES_COPY.md](./PX_V1_3_1_P1_HYPOTHESES_STRUCTURANTES_COPY.md) | active | P1 — Hypothèses structurantes (atterrissage + première vérif) |
| [PX_V1_3_1_P1_SYSTEM_SCALPELS_COPY.md](./PX_V1_3_1_P1_SYSTEM_SCALPELS_COPY.md) | active | P1 System Scalpels Copy v1.3.1 |
| [PX_V1_3_1_P2_COPY.md](./PX_V1_3_1_P2_COPY.md) | draft | PX V1.3.1 — P2 Copy Pack |
| [PX_V1_3_RESOURCES_CATALOG_V1.json](./PX_V1_3_RESOURCES_CATALOG_V1.json) | active | Fichier JSON |
| [PX_V1_3_RESOURCES_LIBRARY_SPEC.md](./PX_V1_3_RESOURCES_LIBRARY_SPEC.md) | active | PX_V1_3_RESOURCES_LIBRARY_SPEC |

**Owners** : Talia

<!-- GENERATED:END -->

---

**Parent** : [20-product_specs/README.md](../README.md)
