// P1 V1.3 — Hypothèses structurantes (front-only)
// Ne jamais logger ces textes avec des données utilisateur, ne jamais les envoyer au backend.

type P1Band = 'very_low' | 'low' | 'medium' | 'high' | 'very_high';
type P1Confidence = 'confirmed' | 'clarified' | 'protected' | 'unclear';

export type P1HypothesisTrigger = {
  blockId: 'B1' | 'B3';
  themeId: string;
...
export type P1Hypothesis = {
  id: string;
  title: string;
  body: string;
  firstCheck: string;
  triggers: P1HypothesisTrigger[];
};

export const P1_HYPOTHESES: P1Hypothesis[] = [
  {
    id: 'mission_cash_arbitrage',
    title: 'Mission ↔ cash : l’arbitrage n’est pas tenu',
    body: `Quand la mission et le financement tirent dans des directions différentes, la structure se met à “tenir au jour le jour”.
...
Résultat fréquent : décisions contestées, épuisement, et sentiment d’injustice.`,
  ...
  {
    id: 'gouvernance_floue',
    title: 'Gouvernance floue : qui décide quoi n’est pas clair',
    body: `Ici, le problème n’est pas “les gens” : c’est l’absence de cadre stable.
...
La structure brûle de l’énergie à interpréter, se justifier, et contester au lieu d’avancer.`,
  ...
  {
    id: 'coulisses_et_opacite',
    title: 'Coulisses : la décision réelle n’est pas traçable',
    body: `Quand l’info circule en privé et que les décisions se prennent “ailleurs”, la structure devient instable.
...
C’est une mécanique classique : l’opacité fabrique du conflit, puis le conflit renforce l’opacité.`,
  ...
  },
  {
    ...
    body: `Une structure peut sembler “fonctionner”, jusqu’au jour où une dépendance critique saute :
...
Là, tout se dérègle d’un coup : urgence, tensions, reproches, décisions précipitées.`,
  ...
  {
    id: 'charge_capacite_reelle',
    title: 'Capacité réelle non pilotée : la structure s’épuise',
    body: `Quand la charge n’est pas mesurée et discutée, la structure glisse vers un mode pompier.
...
À terme : désengagement, absentéisme, turnover, ou explosion d’un conflit “de trop”.`,
  ...
  {
    id: 'controles_faciles_a_contourner',
    title: 'Contrôles contournables : la fraude/incident est “facile”',
    body: `Si la validation est floue, la traçabilité faible, et les exceptions fréquentes,
