...
import { p2PanoramaQuestions, P2_PANORAMA_AXIS_ORDER, type P2PanoramaAxisId } from '~/config/journeys/p2QuestionsV1_0';

const props = defineProps<{
  goToStep: (stepId: string) => void;
}>();

const copy = p2Copy.panorama;
const questions = p2PanoramaQuestions;
const axisOrder = P2_PANORAMA_AXIS_ORDER;
const skipNoticeId = 'p2-skip-notice-panorama';
...
const axisTotals = axisOrder.reduce<Record<P2PanoramaAxisId, number>>((acc, axisId) => {
  acc[axisId] = questions.filter((q) => q.axisId === axisId).length;
  return acc;
}, {} as Record<P2PanoramaAxisId, number>);
