<template>
  <JourneyLayout>
    <div class="pp-journey-panel space-y-4" role="region" aria-labelledby="journey-step-heading-B3_bilan">
      <JourneyStepHeader
        :title="blockCopy.bilanTitle"
        :subtitle="blockCopy.bilanSubtitle || blockCopy.subtitle"
        heading-id="journey-step-heading-B3_bilan"
      />
      <div v-if="hasScores" class="pp-journey-section space-y-3">
        <section class="space-y-2" aria-labelledby="b3-summary-heading">
          <h2 id="b3-summary-heading" class="pp-journey-title text-lg">
        ...
        <section class="space-y-2" aria-labelledby="b3-interpretation-heading">
          <h2 id="b3-interpretation-heading" class="pp-journey-title text-lg">
...
const narrative = computed(() => ({
  summary:
    narrativeResult.value.summary && narrativeResult.value.summary.length
      ? narrativeResult.value.summary
      : narrativeResult.value.summaryKeys.map((key) => ({
          key,
          text: p1PanoramaText[key] ?? key
        })),
  interpretation:
    narrativeResult.value.interpretation && narrativeResult.value.interpretation.length
      ? narrativeResult.value.interpretation
      : narrativeResult.value.interpretationKeys.map((key) => ({
          key,
          text: p1PanoramaText[key] ?? key
        }))
}));
