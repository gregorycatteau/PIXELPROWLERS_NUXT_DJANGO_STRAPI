#!/usr/bin/env bash
# Usage: bash frontend_nuxt/scripts/smoke/smoke-p4-value.sh
set -euo pipefail
...
TARGET_PATH="/parcours/parcours-p4"
...
NODE_ENV=production npm run build >/tmp/pp-smoke-p4-build.log 2>&1
NITRO_PORT="$PORT" node .output/server/index.mjs >/tmp/pp-smoke-p4-prod.log 2>&1 &
PROD_PID=$!
...
if [[ "$status" != "200" ]]; then
  echo "❌ smoke-p4-value: expected 200 for ${TARGET_PATH}, got ${status}"
  exit 1
fi

TSX_CODE=$(cat <<'NODE'
import { p4BilanAdapter } from './app/adapters/bilan/p4';
...
const vm = p4BilanAdapter.buildViewModel();
...
recommendations.forEach((reco) => {
  ...
  if (!journeys.includes('p4')) {
    throw new Error(`resource ${reco.slug} not tagged p4`);
  }
...
echo "✅ smoke-p4-value: ${TARGET_PATH} -> OK"
